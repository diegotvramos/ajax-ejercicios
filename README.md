...
###  WordPress REST API & Infinite Scroll (5 / 5)

El código que hagamos para generar los post infinitos no solo te va servir para wordpres, practicamente cualquier api que te dé la url hacia la siguiente página(grupo de resultados) o una página previa.

CSS TRICKS
Hay un elemento que dentro del embedded no traia el objeto wp:featuredmedia, y al no traer.


¿Te acuerdas cuando consultabamos la api de los shows de televicion que por ahi todo los shows no tenian poster promocional? y pues marcaba error por que no venia ese parámetro que tratábamos de imprimir? 

¿Como yo podria validar esto?

Lo solucionamos con un operador ternario.

```javascript
    $template.querySelector(".post-image").src=el._embedded["wp:featuredmedia"]?el._embedded
            ["wp:featuredmedia"][0].source_url:"";
```
y de preferencia tambien tendrian que hacer a lo mejor con las categorias y las etiquetas.

Para hacer el Scroll Infitito debemos irnos a la documentacion de wordPress.

> `https://developer.wordpress.org/rest-api/reference/posts/` `https://developer.wordpress.org/rest-api/reference/posts/#list-posts`

![paginacion](/assets/paginacion.JPG)

vamos atrabajar con esas 2 propiedades, esas 2 propiedades debemos modificarcelas a la API

```javascript
    POSTS= `${API_WP}/posts?_embed&per_page=5&page=10`,
```
necesito declarar un evento para la ventana.

Traten de hacer algo pensando en que si va escalar pueda seguir funcionando, por eso mejor creo variables

El escroll infinito va depender de la api que estés consumiendo y muy importante debes de leer la documentacion así como en el ejercicio de los pokemones
...
Siguiente: SPA (reack vue.js sbeld, angular) mantener un estado, cargar toda la interfaz via js generalmente en este tipo de librerias tenemos el DOM con un ID en el html y todo se carga dinamicamente, y manejar ciertas rutas que puedan ser reales a travez del hash, componetizar, paso de parametros
